...
class Tester:
    ...
    async def get_from_tx(self, tx_id: int) -> dict:
        ...
        delta_req = indy_vdr.ledger.build_get_revoc_reg_delta_request(submitter_did=None, revoc_reg_id=rev_reg_id, from_ts=None, to_ts=timestamp)
        res = await self.pool.submit_request(delta_req)
        return res
